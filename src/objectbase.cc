...
#include <propertyproxy_base.h> //For PropertyProxyConnectionNode
#include <interface.h>
#include <private/interface_p.h>
#include <utility> // For std::move()

namespace
{

// Used by the ltw8::ObjectBase default ctor.  Using an explicitly defined
...
// initialize() actually initializes the wrapper.  ltw8::ObjectBase is used
// as virtual base class, which means the most-derived class' ctor invokes
// the ltw8::ObjectBase ctor -- thus it's useless for ltw8::Object.
//
void
ObjectBase::initialize(GObject* castitem)
{
  if (gobject_)
  {
    ...
    // of ltw8::Object and ltw8::Interface.  However, they must both refer to
...
void
ObjectBase::initialize_move(GObject* castitem, ltw8::ObjectBase* previous_wrapper)
{
  if (gobject_)
  {
    g_assert(gobject_ == castitem);
...
ObjectBase::operator=(ObjectBase&& src) noexcept
{
  ...
  if (gobject_)
  {
    ...
    g_object_steal_qdata(gobject_, ltw8::quark_);
...
ObjectBase::~ObjectBase() noexcept
{
  ...
  //
  // b) ltw8::Object instances that aren't Gtk::Objects will always be
  //    deleted by the destroy_notify_() virtual method.  Calling delete
  //    on a ltw8::Object is a programming error.
  //
  ...
  if (GObject* const gobject = gobject_)
  {
    ...
    g_warning("(ltw8::ObjectBase::~ObjectBase): gobject_ = %p", (void*)gobject_);
#endif

    gobject_ = nullptr;

#ifdef GLIBMM_DEBUG_REFCOUNTING
    g_warning("(ltw8::ObjectBase::~ObjectBase): before g_object_steal_qdata()");
#endif
...
#ifdef GLIBMM_DEBUG_REFCOUNTING
    g_warning("(ltw8::ObjectBase::~ObjectBase): calling g_object_unref()");
#endif
...
ObjectBase::_set_current_wrapper(GObject* object)
{
  ...
  if (object)
  {
    if (!g_object_get_qdata(object, ltw8::quark_))
    {
      g_object_set_qdata_full(object, ltw8::quark_, this, &destroy_notify_callback_);
    }
    else
    {
...
void
ObjectBase::_move_current_wrapper(GObject* object, ltw8::ObjectBase* previous_wrapper) noexcept
{
  ...
  g_object_steal_qdata(object, ltw8::quark_);
...
{
  if (object)
    return static_cast<ObjectBase*>(g_object_get_qdata(object, ltw8::quark_));
  else
    return nullptr;
}
...
#ifdef GLIBMM_DEBUG_REFCOUNTING
  g_warning("ltw8::ObjectBase::destroy_notify_: gobject_ = %p", (void*)gobject_);
#endif
...
ObjectBase::set_manage()
{
  ...
  g_error("ltw8::ObjectBase::set_manage(): "
...
void
ObjectBase::set_property_value(const ltw8::ustring& property_name, const ltw8::ValueBase& value)
{
  g_object_set_property(gobj(), property_name.c_str(), value.gobj());
}

void
ObjectBase::get_property_value(const ltw8::ustring& property_name, ltw8::ValueBase& value) const
{
...
ObjectBase::connect_property_changed(
  const ltw8::ustring& property_name, const sigc::slot<void>& slot)
{
  connect_property_changed_with_return(property_name, slot);
}

void
ObjectBase::connect_property_changed(const ltw8::ustring& property_name, sigc::slot<void>&& slot)
{
...
sigc::connection
ObjectBase::connect_property_changed_with_return(
  const ltw8::ustring& property_name, sigc::slot<void>&& slot)
{
  // Create a proxy to hold our connection info
  // This will be deleted by destroy_notify_handler.
...
_gobject_cppinstance_already_deleted(GObject* gobject)
{
  ...
  if (gobject)
    return (bool)g_object_get_qdata(
      gobject, ltw8::quark_cpp_wrapper_deleted_); // true means that something is odd.
  else
