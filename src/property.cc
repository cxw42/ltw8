...
//    must not be associated with a single ltw8::Property<> instance.  Rather,
...
// ---------------------
// a) Assign an ID to a ltw8::PropertyBase by keeping track of the number of
...
// b) In the given run of a program, all ltw8::Property<> instances related to
//    a given ltw8::Object must be constructed in the same order.
//
// Advantages:
// -----------
...
{
  auto obj_iface_props = static_cast<ltw8::Class::iface_properties_type*>(data);
  if (obj_iface_props)
  {
    for (ltw8::Class::iface_properties_type::size_type i = 0; i < obj_iface_props->size(); i++)
    {
...
using custom_properties_type = std::vector<ltw8::PropertyBase*>;
  ...
  GObject* object, unsigned int property_id, GValue* value, GParamSpec* param_spec)
{
  ...
  {
    if (ltw8::ObjectBase* const wrapper = ltw8::ObjectBase::_get_current_wrapper(object))
    {
      auto obj_custom_props =
...
PropertyBase::PropertyBase(ltw8::Object& object, GType value_type)
...
bool
PropertyBase::lookup_property(const ltw8::ustring& name)
{
...
{
  return ltw8::ustring(get_name_internal());
}

ltw8::ustring
PropertyBase::get_nick() const
{
  return ltw8::convert_const_gchar_ptr_to_ustring(
