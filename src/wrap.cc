...
using WrapFuncTable = std::vector<ltw8::WrapNewFunction>;
...
wrap_register_init()
{
  ...
  if (!ltw8::quark_)
  {
    ltw8::quark_ = g_quark_from_static_string("glibmm__ltw8::quark_");
    ltw8::quark_cpp_wrapper_deleted_ =
      g_quark_from_static_string("glibmm__ltw8::quark_cpp_wrapper_deleted_");
  }
...
wrap_register(GType type, WrapNewFunction func)
{
  ...
  g_type_set_qdata(type, ltw8::quark_, GUINT_TO_POINTER(idx));
}

static ltw8::ObjectBase*
wrap_create_new_wrapper(GObject* object)
{
  ...
  const bool gtkmm_wrapper_already_deleted =
    (bool)g_object_get_qdata((GObject*)object, ltw8::quark_cpp_wrapper_deleted_);
  if (gtkmm_wrapper_already_deleted)
  {
    g_warning("ltw8::wrap_create_new_wrapper: Attempted to create a 2nd C++ wrapper for a C "
  ...
  for (GType type = G_OBJECT_TYPE(object); type != 0; type = g_type_parent(type))
  {
    ...
    //
    if (const gpointer idx = g_type_get_qdata(type, ltw8::quark_))
    {
      const ltw8::WrapNewFunction func = (*wrap_func_table)[GPOINTER_TO_UINT(idx)];
      return (*func)(object);
    }
  }
...
wrap_create_new_wrapper_for_interface(GObject* object, GType interface_gtype)
{
  ...
  for (GType type = G_OBJECT_TYPE(object); type != 0; type = g_type_parent(type))
  {
    ...
    //
    const gpointer idx = g_type_get_qdata(type, ltw8::quark_);
...
{
  return ltw8::RefPtr<Object>(dynamic_cast<Object*>(wrap_auto(object, take_copy)));
}
