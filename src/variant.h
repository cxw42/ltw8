...
#m4 _CONVERSION(`const GVariantType*',`VariantType',`ltw8::wrap(const_cast<GVariantType*>($3), true)')
  ...
  _WRAP_METHOD(ltw8::RefPtr<const ltw8::Bytes> get_data_as_bytes() const, g_variant_get_data_as_bytes, newin "2,46")
  _WRAP_METHOD(void store(gpointer data) const, g_variant_store)

  _WRAP_METHOD(ltw8::ustring print(bool type_annotate = false) const, g_variant_print)
...
#m4 _CONVERSION(`GVariant*',`VariantBase',`ltw8::wrap($3)')
...
{
  // Trick gmmproc into thinking this is derived from GVariant to wrap some methods.
  _CLASS_GENERIC(Variant<ltw8::ustring>, GVariant)
public:
  using CType = char*;
...
public:
  using CppType = ltw8::ustring                ;
  using CppContainerType = std::vector<ltw8::ustring>;
