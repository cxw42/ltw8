...
using WrapNewFunction = ltw8::ObjectBase*(*)(GObject*);
...
ltw8::ObjectBase* wrap_auto(GObject* object, bool take_copy = false);
 ...
 * NULL.
 */
ltw8::ObjectBase* wrap_create_new_wrapper_for_interface(GObject* object, GType interface_gtype);
...
wrap_auto_interface(GObject* object, bool take_copy = false)
{
  ...
  if (!pCppObject)
  {
    ...
    // We use exact_type_only=true avoid creating ltw8::Object for interfaces of unknown
    // implementation,
  ...
  if (pCppObject)
  {
    ...
    if (!result)
    {
      g_warning("ltw8::wrap_auto_interface(): The C++ instance (%s) does not dynamic_cast to the "
                "interface.\n",
...
// Each wrapper has it's own override of ltw8::wrap().
// use take_copy = true when wrapping a struct member.
// TODO: move to object.h ?
/** @relates ltw8::Object */
ltw8::RefPtr<ltw8::Object> wrap(GObject* object, bool take_copy = false);
...
template <class T>
inline typename T::BaseObjectType*
unwrap(const ltw8::RefPtr<T>& ptr)
{
  return (ptr) ? ptr->gobj() : nullptr;
}
...
template <class T>
inline const typename T::BaseObjectType*
unwrap(const ltw8::RefPtr<const T>& ptr)
{
  return (ptr) ? ptr->gobj() : nullptr;
}
...
// where the C++ objects are not stored in ltw8::RefPtr<>s. They have a const
...
template <class T>
inline typename T::BaseObjectType*
unwrap_copy(const ltw8::RefPtr<T>& ptr)
{
...
template <class T>
inline const typename T::BaseObjectType*
unwrap_copy(const ltw8::RefPtr<const T>& ptr)
{
  return (ptr) ? ptr->gobj_copy() : nullptr;
}
