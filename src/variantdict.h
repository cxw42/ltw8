...
public:
  _WRAP_METHOD(static ltw8::RefPtr<VariantDict> create(const VariantBase& from_asv{?}), g_variant_dict_new)
...
//just returning a VariantBase that should be cast_dynamic()ed?
  ...
  _WRAP_METHOD(bool contains(const ltw8::ustring& key) const, g_variant_dict_contains)

  _IGNORE(g_variant_dict_insert)


  _WRAP_METHOD(void insert_value_variant(const ltw8::ustring& key, const VariantBase& value), g_variant_dict_insert_value)
  ...
  _WRAP_METHOD(bool remove(const ltw8::ustring& key), g_variant_dict_remove)
...
{
  using type_glib_variant = ltw8::Variant<T_Value>;
