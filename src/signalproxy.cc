...
SignalProxyBase::SignalProxyBase(ltw8::ObjectBase* obj) : obj_(obj)
{
}

// SignalProxyNormal implementation:

SignalProxyNormal::SignalProxyNormal(ltw8::ObjectBase* obj, const SignalProxyInfo* info)
...
SignalProxyNormal::slot0_void_callback(GObject* self, void* data)
{
  ...
  if (ltw8::ObjectBase::_get_current_wrapper(self))
  {
    try
    {
...
: SignalProxyBase(obj),
  info_(info),
  detailed_name_(ltw8::ustring(info->signal_name) +
                 (detail_name.empty() ? ltw8::ustring() : ("::" + detail_name)))
{
}
