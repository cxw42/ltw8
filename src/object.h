...
#include <quark.h>
#include <refptr.h>
#include <utility.h> /* Could be private, but that would be tedious. */
#include <containerhandle_shared.h> /* Because its specializations may be here. */
#include <value.h>
 ...
 * is that the actual construction is always done in the ltw8::Object ctor.
...
{
public:
  const ltw8::Class& glibmm_class;
  unsigned int n_parameters;
  GParameter* parameters;

  explicit ConstructParams(const ltw8::Class& glibmm_class_);
  ConstructParams(const ltw8::Class& glibmm_class_, const char* first_property_name,
...
protected:
  ...
  explicit Object(const ltw8::ConstructParams& construct_params);
...
// static RefPtr<Object> create(); //You must reimplement this in each derived class.
  ...
  // ltw8::Objects contain a list<Quark, pair<void*, DestroyNotify> >
...
private:
  friend class ltw8::Object_Class;
  static CppClassType object_class_;

#endif /* DOXYGEN_SHOULD_SKIP_THIS */

  // ltw8::Object can not be dynamic because it lacks a float state.
