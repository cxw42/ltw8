...
{
  return (str) ? ltw8::ustring(ltw8::make_unique_ptr_gfree(str).get()) : ltw8::ustring();
}
...
{
  return (str) ? std::string(ltw8::make_unique_ptr_gfree(str).get()) : std::string();
}

/** Get a pointer to the C style string in a std::string or ltw8::ustring.
